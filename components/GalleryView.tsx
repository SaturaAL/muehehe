...
function FigmaIcon() {
  return (
    ...
    >
      ...
      <path
        d="M12 2h4a3 3 0 1 1 1 3 3 3 0 0 1-1 3h-4V2Z"
        fill="currentColor"
      />
      <path
        d="M8 8h4v6H8a3 3 0 1 1 0-6Z"
        fill="currentColor"
      />
      <path
        d="M12 8h4a3 3 0 1 1 0 0 6 0v-6Z"
        fill="currentColor"
      />
      <path
        d="M8 14h4v6a3 3 0 1 1-3-3 3 3 0 0 1-1-3Z"
        fill="currentColor"
      />
    </svg>
  );
...
export default function GalleryView({ posts }: { posts: Post[] }) {
  ...
  const extraImages = Array.isArray(selected?.images)
    ? (selected.images as string[])
    : [];

  const slides = selected
    ? [selected.imageUrl, ...extraImages]
    : [];
  ...
  return (
    <div>
      ...
      <div className="mb-8 flex items-center gap-2 border-b border-[#2B2622]/10">
        {(["ILLUSTRATION", "PROGRAMMING"] as const).map(
          (tab) => (
            <button
              key={tab}
              ...
              className={`relative px-5 py-3 text-sm font-medium transition-all duration-300 ${
                activeTab === tab
                  ? "text-[#2B2622]"
                  : "text-[#2B2622]/40 hover:text-[#2B2622]/70"
              }`}
            >
              {tab === "ILLUSTRATION"
                ? "Illustration"
        ...
        {filtered.length === 0 && (
          <div className="col-span-full flex min-h-40 items-center justify-center">
            <div className="text-center">
              <p className="font-script text-2xl text-[#DD8C8C]/70">
                Belum ada karya...
              </p>

              <p className="mt-1 text-xs text-[#2B2622]/30">
                Coba cek kategori lainnya.
              </p>
            </div>
          </div>
        )}
